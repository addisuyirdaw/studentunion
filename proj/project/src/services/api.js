const API_BASE_URL =' https://studentunion-dcjr.onrender.com/api';
...
class ApiService {
  ...
  async getClubActivities(id) {
    const response = await this.request(`/clubs/${id}/activities`);
    return response.activities || [];
  }

  async getAllClubReports() {
    const response = await this.request('/clubs/reports/all');
    return response.reports || [];
  }

  async createClubActivity(id, activityData) {
    const isFormData = activityData instanceof FormData;
    return this.request(`/clubs/${id}/activities`, {
      method: 'POST',
      body: isFormData ? activityData : JSON.stringify(activityData),
      isFormData
    });
  }
  ...
  async assignRep(studentId, clubId) {
    return this.request('/users/assign-rep', {
      method: 'PATCH',
  ...
  async getUserByStudentId(studentId) {
    const response = await this.request(`/users/student/${studentId}`);
