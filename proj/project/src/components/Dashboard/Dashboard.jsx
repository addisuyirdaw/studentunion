...
export function Dashboard() {
	...
	return (
		<div className="space-y-6">
			...
			{/* Security Alert Banner for CLUB_REP */}
			{user?.role === 'CLUB_REP' && (
				<motion.div
					initial={{ opacity: 0, y: 10 }}
					animate={{ opacity: 1, y: 0 }}
					className="bg-amber-100 border border-amber-400 text-amber-800 px-4 py-3 rounded-xl flex items-center shadow-sm">
					<AlertTriangle className="w-6 h-6 mr-3 text-amber-600 flex-shrink-0" />
					<div>
						<h4 className="font-bold">Security Alert</h4>
						<p className="text-sm mt-1">Please change your password to secure your club management tools.</p>
					</div>
				</motion.div>
			)}
